...
mounted on directory *b*, thus allowing access to files */b/x* and */b/y*. If directory *b* had contained any files they would not be accessible while the CD-ROM was
...
kinds of special files exist: **block special files** and **character special** files. Block special files are used to model devices that consist of a collection of randomly addressable blocks, such as disks. By opening a block special file and reading, say,
...
using a graphical user interface. Many shells exist, including *sh, csh, ksh*, and *bash*. All of them support the functionality described below, which derives from the original shell *(sh)*.
...
arrange all the lines in alphabetical order. The output of sort is redirected to the file */dev/lp*, typically the printer.
...
with each file is a pointer that indicates the current position in the file. When reading (writing) sequentially, it normally points to the next byte to be read (written). The lseek call changes the value of the position pointer, so that subsequent calls to
...
#### **1.6.3 System Calls for Directory Management**
 ...
 the file system as a whole, rather than just to one specific file as in the previous
...
i-number is an index into a table of **i-nodes**, one per file, telling who owns the file, where its disk blocks are, and so on. A directory is simply a file containing a set of (i-number, ASCII name) pairs. In the first versions of UNIX, each directory entry
...
### **1.9 RESEARCH ON OPERATING SYSTEMS**
 ...
 going. Researchers at universities and industrial research labs are constantly
